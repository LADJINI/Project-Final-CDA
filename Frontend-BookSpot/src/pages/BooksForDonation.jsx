...
const BooksForDonation = () => {
  const { booksToGive, setBooksToGive } = useBooks();
  const { user } = useAuth();
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      if (!user || !user.id) {
        console.error('Utilisateur non connecté ou nom d\'utilisateur manquant');
        return;
      }
      try {
        const response = await axios.get(`http://localhost:8086/api/books/user/${user.id}/type/3`, {
          headers: {
  ...
  const confirmDelete = async () => {
    try {
      ...
      setBooksToGive(booksToGive.filter(book => book.id !== bookToDelete.id));
  ...
  const handleUpdate = async (updatedBookData) => {
    try {
      ...
      setBooksToGive(booksToGive.map(book => (book.id === response.data.id ? response.data : book)));
      setIsEditing(false);
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 border rounded-md shadow-lg bg-white">
      <h1 className="text-2xl font-bold mb-4">Livres à donner</h1>
...
export default BooksForDonation;